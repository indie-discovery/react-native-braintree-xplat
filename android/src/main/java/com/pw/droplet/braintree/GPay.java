...
public class GPay {
                ...
                if (json != null) {
                  ...
                  try {
                    ...
                    if (paymentMethodData != null) {
                      Log.v("Token : ", token);
                      Log.e("Sarju", paymentMethodData.toString());
                      Log.v("Response", paymentMethodData.toString());
                      mRequestPaymentPromise.resolve(convertJsonToMap(paymentMethodData));
                    }
                  } catch (JSONException e) {
                    mRequestPaymentPromise.reject(E_PAYMENT_DATA, e.getMessage());
                  }
  ...
  private static JSONObject getBaseCardPaymentMethod(ReadableArray cardNetworks) throws JSONException {
    ...
    cardPaymentMethod.put("parameters", new JSONObject().put("allowedAuthMethods", GPay.getAllowedCardAuthMethods())
  ...
  private static JSONObject getIsReadyToPayRequest(ReadableArray cardNetworks) {
    try {
      ...
      isReadyToPayRequest.put("allowedPaymentMethods", new JSONArray().put(getBaseCardPaymentMethod(cardNetworks)));
   ...
   */
  @ReactMethod
  public void show(int environment, ReadableMap requestData, final Promise promise, @NonNull Activity activity) {
    if (activity == null) {
      promise.reject(E_NO_ACTIVITY, "activity is null");
      return;
    }
